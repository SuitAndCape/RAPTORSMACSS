...
var chalk = gutil.colors,
    ...
    green = chalk.green,
    red = chalk.red,
    highlight = chalk.cyan.underline.bgBlue,
    log = gutil.log,
    src = gulp.src,
...
gulp.task('raptor-style', function() {
  log('Generating', highlight('RAPTORSMACSS'), 'based', red('stylesheets'), 'at', green(timestamp));
  // gulp.series('compile-styles', 'minify-styles');
  runSequence('compile-styles', 'minify-styles');
});

gulp.task('raptor-script', function() {
  log('Generating', highlight('RAPTORSMACSS'), 'based', red('JavaScript'), 'files at', green(timestamp));
